...
const Sidebar = () => {
  // State to track the selected button and conference data
        ...
        <div className="overflow-y-auto py-5 px-3 h-full bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700">
          <ul className="space-y-2">
            ...
            <li>
              ...
              {/* Render conference list if the "Conference" button is selected */}
              {selectedButton === 'conference' && (
                <ul className="pl-6 mt-2">
                  {/* Render conference items */}
                  {conferences.map((conference, index) => (
                    <li key={index} className="py-1">
                      <button type="button" className="text-left text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 w-full p-1 rounded-lg transition">
                        {conference.name}
                      </button>
                    </li>
                  ))}
